                                ...
                                <div class="table-responsive">
                                    <table id="tbl_items" style="overflow: scroll;border:1px solid black;" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-font:tahoma;">
                                        ...
                                        <tr>
                                            ...
                                            <th width="10%">Qty</th>
                                            <th >Description</th>
                                            <th width="12%" style="text-align: right">Redeem</th>
                                        ...
                                        <tr>
                                            <td colspan="3" style="text-align: right;"><strong><i class="glyph-icon icon-star"></i> Total Redeem Points :</strong></td>
...
$(document).ready(function(){
    ...
    var oItemEnlistment; var oTableHistory; var dualDisplayHandle;

    var oTableItems={

        qty : 'td:eq(0)',
        required_points_redeem : 'td:eq(2)',
        total : 'td:eq(3)',
        item_id : 'td:eq(4)'
    ...
    var initializeControls=function(){
        oItemEnlistment=$('#tbl_items_enlistment').DataTable({
            ...
            "bLengthChange":false,
            "iDisplayLength" : 5,
            "ajax" : "Redeem_points/transaction/items-list/",
            "columns": [

                { targets:[0],data: "item_code" },
                { targets:[1],data: "item_name" },
                { targets:[2],data: "required_points_redeem" },
                {
                    targets:[3],
        ...
        oTableHistory=$('#tbl_history').DataTable({
            ...
            "bLengthChange":false,
            "iDisplayLength" : 5,
            "ajax" : "Redeem_points/transaction/list",
            "columns": [

                { targets:[1],data: "red_txn_no" },
    ...
    var bindEventHandlers=function(){
        ...
        $('.tile_box_menu').click(function(){
            ...
            var redeem_points = $(this).data('item-points');

            $('#tbl_items > tbody').prepend(newRowItem({
                item_qty : "1",
                item_name : item_name,
                redeem_points : redeem_points,
                pt_line_total : 1*redeem_points,
                item_id: item_id
            }));
        ...
        $('#tbl_items tbody').on('keyup','input.numeric',function(){
            ...
            var required_points_redeem=parseFloat(accounting.unformat(row.find(oTableItems.required_points_redeem).find('input.numeric').val()));
            var qty=parseFloat(accounting.unformat(row.find(oTableItems.qty).find('input.numeric').val()));
            var line_total=required_points_redeem*qty;
        ...
        $('#tbl_items_enlistment > tbody').on('click','button[name="add_item"]',function(){
            ...
            var redeem_points = data.required_points_redeem;
            ...
            if(event.keyCode==13){
                ...
                if(validateRequiredFields(f)){ //if all required fields are supplied, continue for ajax post request
                    ...
                    $.ajax({
                        "dataType":"json",
                        "type":"POST",
                        "url":"Redeem_points/transaction/create",
                        "data":_data
                        ...
                        if(response.stat=="success"){
                            ...
                            if(dualDisplayHandle!=null&&dualDisplayHandle!=undefined){
                                ...
                                dualDisplayHandle.$('#header_statement').html("You redeem ");
    ...
    var clearFields=function(f){
        ...
        $('#tbl_items > tbody').html('');
        showAdvertisement(8000);showAdvertisement(8000);
    };
    ...
    var newRowItem=function(d){
        ...
        '<td width="11%"><input name="redeem_points[]" type="text" class="numeric form-control" value="'+accounting.formatNumber(d.redeem_points,2)+'" style="text-align:right;"></td>'+
